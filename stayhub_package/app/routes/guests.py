...
def delete_guest(guest_id: int, service: GuestService = Depends(get_guest_service)):
    ...
    return {"message": "Гость успешно удалён"}
...
def guests_list(
    request: Request,
    q: Optional[str] = Query(None, description="Поиск по гостям"),
    sort: str = Query("name", description="Поле сортировки"),
    direction: str = Query("asc", description="Направление сортировки"),
    service: GuestService = Depends(get_guest_service)
):
    """Guests list page with search and sorting."""
    allowed_sort_fields = {"name", "email", "phone", "address", "document"}
    sort_field = sort.lower() if sort and sort.lower() in allowed_sort_fields else "name"
    sort_direction = direction.lower() if direction and direction.lower() in {"asc", "desc"} else "asc"

    guests = service.list_guests(search=q, sort_field=sort_field, sort_direction=sort_direction)
